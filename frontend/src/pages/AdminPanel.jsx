  ...
  const handleGenerateStory = async () => {
    ...
    try {
      const response = await axios.post(
        ...
        {
          text: `Generate a ${form.genre} folktale from ${form.region} suitable for ${form.ageGroup}. The story should be engaging, culturally relevant, and appropriate for the selected age group. Provide a title and the story content.`,
        },
        {
          headers: {
            'Api-Key': 'e2c51089-0956-49fb-9bd2-4a34c2d375bd',
      ...
      // Parse the generated text to extract title and content (assuming DeepAI returns title and story in a structured format)
      const titleMatch = generatedText.match(/Title:\s*(.*?)\n/);
      const title = titleMatch ? titleMatch[1].trim() : `A ${form.genre} Tale from ${form.region}`;
      const content = generatedText.replace(/Title:\s*.*?\n/, '').trim();

      setForm({
        ...form,
        title: form.title || title, // Only set title if not already set
        content: content,
      });
